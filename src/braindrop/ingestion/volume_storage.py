...
def ensure_volume_exists(
    ...
    logger.info(f"Ensuring volume '{catalog}.{schema}.{volume}' exists...")
    ...
    logger.info(f"Volume {catalog}.{schema}.{volume} is ready.")
...
def save_to_volume(
    ...
    try:
        ...
        logger.warning(
            f"Standard I/O failed for {volume_path}: {e}. Trying WorkspaceClient..."
        )
        try:
            w = WorkspaceClient()
            ...
            logger.error(f"WorkspaceClient upload failed: {sdk_e}")
